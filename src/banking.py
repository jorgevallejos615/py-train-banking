...
class BankAccount:
    ...
    def convert_currency(
        ...
        converted_balance = (self.__balance * validated_rate).quantize(Decimal("0.01"))
    ...
    def create_savings(
        ...
        if _account_type == "savings" and validated_balance < Decimal("100.00"):
            raise ValueError(
                "Savings accounts require an initial balance of at least 100.00"
            )
    ...
    def from_string(cls, account_data: str) -> BankAccount:
        ...
        if len(parts) != 4:
            raise ValueError(
                "Account data must contain number, currency, type, and balance"
            )
